...
def initialize_session_state():
    ...
    if "active_page" not in st.session_state:
        st.session_state.active_page = "Financial Health Calculator"

# =========================================
# TOP NAVBAR (native)
# =========================================
def render_top_nav(pages):
    st.markdown("""
    <style>
      .fyn-nav { position: sticky; top: 0; z-index: 1000; background: white;
                 padding: 8px 10px; border-bottom: 1px solid #e5e7eb; }
      .fyn-nav-row { display: flex; gap: 8px; }
      .fyn-pill { padding: 6px 14px; border: 1px solid #e5e7eb; border-radius: 9999px;
                  font-weight: 600; cursor: pointer; }
      .fyn-pill.active { background:#0f172a; color:#fff; border-color:#0f172a; }
    </style>
    """, unsafe_allow_html=True)

    with st.container():
        st.markdown("<div class='fyn-nav'><div class='fyn-nav-row'>", unsafe_allow_html=True)
        cols = st.columns(len(pages))
        for i, p in enumerate(pages):
            is_active = (st.session_state.active_page == p)
            # Button triggers page change; pill style shown below it
            if cols[i].button(p, key=f"nav_btn_{p}"):
                st.session_state.active_page = p
                st.rerun()
            cols[i].markdown(f"<div class='fyn-pill {'active' if is_active else ''}'>{p}</div>", unsafe_allow_html=True)
...
def render_floating_chat(ai_available, model):
    ...
    <style>
      ...
      .fynyx-fab-wrap .stButton>button {
        border-radius: 9999px; padding: 10px 14px; font-weight: 600;
        box-shadow: 0 6px 16px rgba(0,0,0,0.15);
      }
      .fynyx-chat-panel {
        position: fixed; right: 20px; bottom: 80px; width: min(420px, 94vw);
        max-height: min(70vh, 680px); background: white; border: 1px solid #e5e7eb;
        border-radius: 14px; box-shadow: 0 12px 30px rgba(0,0,0,0.18);
        overflow: hidden; z-index: 9999;
      }
      .fynyx-chat-header {
        padding: 10px 12px; background: #0f172a; color: #fff; font-weight: 600;
        display: flex; align-items: center; justify-content: space-between;
      }
      .fynyx-chat-body { padding: 10px 12px 8px 12px; max-height: 50vh; overflow-y: auto; }
      .fynyx-msg { margin: 8px 0; padding: 10px 12px; border-radius: 10px; font-size: 0.92rem; line-height: 1.3rem; }
      .fynyx-user { background: #eef2ff; }
      .fynyx-bot { background: #f1f5f9; }
      .fynyx-meta { font-size: 0.74rem; color: #64748b; margin-top: 4px; }
      .fynyx-chat-footer { padding: 8px 10px 12px 10px; border-top: 1px solid #e5e7eb; background: #fff; }
      @media (max-width: 480px) { .fynyx-chat-panel { bottom: 70px; right: 10px; width: 94vw; } }
    </style>
    """, unsafe_allow_html=True)

    # Floating button (we place a normal Streamlit button, then position its wrapper via HTML anchor)
    fab_anchor = st.empty()
    with fab_anchor.container():
        if st.button(("💬 Chat with FYNyx" if not st.session_state.chat_open else "✖ Close chat"),
                     key="fyn_fab_btn", help="Ask about savings, investing, debt, etc."):
            st.session_state.chat_open = not st.session_state.chat_open
            st.rerun()
    # Move the above container to a fixed spot
    st.markdown("<div class='fynyx-fab-wrap'></div>", unsafe_allow_html=True)
    ...
    if st.session_state.chat_open:
        ...
        st.markdown(
            ...
            f"<span style='font-size:12px;opacity:.9;'>{'AI online' if ai_available else 'Basic mode'}</span>"
            "</div>",
            unsafe_allow_html=True
        )
        ...
        if not history:
            ...
            for chat in history:
                st.markdown(f"<div class='fynyx-msg fynyx-user'><b>You:</b> {chat['question']}</div>", unsafe_allow_html=True)
                st.markdown(
                    f"<div class='fynyx-msg fynyx-bot'><b>FYNyx:</b> {chat['response']}"
                    f"<div class='fynyx-meta'>{'🤖 AI' if chat.get('was_ai_response') else '🧠 Fallback'} • {chat['timestamp']}</div>"
                    f"</div>",
                    unsafe_allow_html=True
                )
...
PAGES = ["Financial Health Calculator", "Goal Tracker"]
